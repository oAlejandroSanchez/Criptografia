...
def __main__():
    ...
    coprimo = int(round(det))%37
    print("\tPor lo tanto, K mod 37 = " + str(coprimo))

    if int(round(det))%37 != 0:
        print("\tLa matriz tiene inversa con módulo 37...")
        print("\tPor lo tanto, el coprimo de K = " + str(int(round(det))) + " es " + str(coprimo))
    else:
        print("\tLa matriz no es inversa en módulo 37")
        return
    ...
    for i in range(len(M)):
        ...
        for j in range(len(M[i])):
            ...
            if j != 1:
                ...
                print("\t" + str(K[j]) + " X " + str(M[i,j]) + " mod(37) = " + str(aux[i,j]))
    ...
    inverso = inverso_multiplicativo(coprimo,37)
    ...
    for i in range(len(aux)):
        ...
        for j in range(len(aux[i])):
            aux2[i,j] = aux2[i,j]%37
            if j != 1:
                print("\t" + str(K[j]) + "   " + str(aux[i,j]) + "           " + str(aux2[i,j]))
            else:
                print("\t" + str(K[j]) + " X " + str(aux[i,j]) + " mod(37) = " + str(aux2[i,j]))
        print()
